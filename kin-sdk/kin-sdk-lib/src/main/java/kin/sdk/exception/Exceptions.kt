 ...
 */
class AccountDeletedException : OperationFailedException("Account deleted, Create new account")

/**
 * Account was not created on the blockchain
 */
class AccountNotFoundException(val accountId: String) : OperationFailedException("Account $accountId was not found")
 ...
 */
class IllegalAmountException(errorReason: String) : OperationFailedException("Illegal amount - $errorReason")

class InsufficientFeeException : OperationFailedException("Not enough fee to perform the transaction.")
/**
 * Transaction failed due to insufficient kin.
 */
class InsufficientKinException : OperationFailedException("Not enough kin to perform the transaction.")

class LoadAccountException(msg: String?, cause: Throwable) : Exception(msg, cause)
...
class TransactionFailedException(
    ...
    companion object {
        private fun getMessage(opResultCode: List<String>?): String {
            return when {
                opResultCode != null && !opResultCode.isEmpty() -> "Transaction failed with the error = " + opResultCode[0]
